...
export function ItemPage({ theme, language, t, setCurrentPage, item }: ItemPageProps) {
  if (!item) {
    return (
      ...
      <div className="mb-6 sm:mb-8">
        <div className="flex items-center gap-4 mb-4 sm:mb-6">
          ...
          <span className={`text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
            | {language === "fr" ? "Détails de la publication" : "Publication details"}
          </span>
        </div>
        ...
        >
          <h1 className="text-xl sm:text-2xl font-semibold mb-3">{item.title}</h1>

          {/* Publication specific info */}
          {item.type === "publication" && (
            <>
              <p className={`mb-2 text-sm sm:text-base ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>
                {item.authors} ({item.year})
              </p>
              <p className={`mb-4 italic text-sm sm:text-base ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                {item.journal}
              </p>
              {item.issn && (
                <p className={`mb-2 text-xs sm:text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                  ISSN: {item.issn}
                </p>
              )}
              {item.category && (
                <p className={`mb-4 text-xs sm:text-sm ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                  Catégorie:{" "}
          ...
          {item.type === "responsibility" && (
            <>
              <p className={`mb-2 text-sm sm:text-base ${theme === "dark" ? "text-gray-400" : "text-gray-600"}`}>
                {item.period}
              </p>
              <p className={`mb-4 text-sm sm:text-base ${theme === "dark" ? "text-gray-300" : "text-gray-700"}`}>
                {item.description}
              </p>
              <div className="space-y-2">
                <h3 className="font-medium text-sm sm:text-base">
                  {language === "fr" ? "Détails des responsabilités:" : "Responsibility details:"}
                </h3>
            ...
            {item.description ? (
              ...
              <div className={`text-center py-8 sm:py-12 ${theme === "dark" ? "text-gray-400" : "text-gray-500"}`}>
                ...
                <p className="text-base sm:text-lg mb-2">
                  {language === "fr" ? "Contenu détaillé à venir" : "Detailed content coming soon"}
                </p>
